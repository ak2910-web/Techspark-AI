...
const Sidebar: React.FC = () => {
  ...
  const startupNavigation: NavSection[] = [
    ...
    },
    {
      title: 'Premium Tools',
      items: [
        { name: 'Pitch Deck Review', path: '/pitch-review', icon: <Star className="w-5 h-5" /> },
        { name: 'Security Settings', path: '/security', icon: <Shield className="w-5 h-5" /> },
      ]
    }
  ];
  ...
  return (
    <motion.aside
      ...
      className={`fixed left-0 top-0 h-screen ${isDarkMode ? 'bg-slate-900 border-slate-800' : 'bg-white border-slate-200'} border-r shadow-sm z-40 flex flex-col`}
    >
      {/* Logo Section */}
      <Link to="/" className={`h-20 flex items-center justify-between px-6 border-b ${isDarkMode ? 'border-slate-800 hover:bg-slate-800' : 'border-slate-200 hover:bg-slate-50'} transition-colors`}>
        <AnimatePresence mode="wait">
          {!collapsed && (
            ...
            >
              <div className={`${isDarkMode ? 'bg-white' : 'bg-black'} p-2 rounded-lg`}>
              ...
              <span className={`text-xl font-bold ${isDarkMode ? 'text-white' : 'text-slate-900'}`}>FundSpark AI</span>
            </motion.div>
          )}
        </AnimatePresence>

        {collapsed && (
          <div className={`${isDarkMode ? 'bg-white' : 'bg-black'} p-2 rounded-lg mx-auto`}>
            <svg className={`w-5 h-5 ${isDarkMode ? 'text-black' : 'text-white'}`} viewBox="0 0 200 200" fill="currentColor">
              <path d="M60 40 L130 40 L185 70 L185 80 L140 80 L140 100 L100 100 L100 80 L65 80 L65 70 Z M80 120 Q80 100 100 100 L120 100 Q140 100 140 120 L140 160 L100 160 L100 140 L80 140 Z M100 140 L120 140 L120 160 L100 160 Z" />
            </svg>
          </div>
      ...
      <button
        ...
        className={`absolute -right-3 top-24 ${isDarkMode ? 'bg-slate-800 border-slate-700 hover:bg-slate-700' : 'bg-white border-slate-200 hover:bg-slate-50'} border rounded-full p-1 shadow-md transition-colors z-50`}
      >
        {collapsed ? (
          <ChevronRight className={`w-4 h-4 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`} />
        ) : (
          <ChevronLeft className={`w-4 h-4 ${isDarkMode ? 'text-slate-300' : 'text-slate-600'}`} />
        )}
      </button>
      ...
      {!collapsed && (
        <div className="px-4 py-4">
          <div className={`${isDarkMode ? 'bg-slate-800' : 'bg-slate-100'} rounded-lg p-1 flex gap-1`}>
            <button
              onClick={() => setActiveSection('startup')}
              className={`flex-1 px-4 py-2 rounded-md font-semibold text-sm transition-all ${
                activeSection === 'startup'
                  ? `${isDarkMode ? 'bg-slate-700' : 'bg-white'} text-[#08D9D6] shadow-sm`
            ...
            <button
              ...
              className={`flex-1 px-4 py-2 rounded-md font-semibold text-sm transition-all ${
                activeSection === 'investor'
                  ? `${isDarkMode ? 'bg-slate-700' : 'bg-white'} text-[#FF2E63] shadow-sm`
                  : `${isDarkMode ? 'text-slate-400 hover:text-slate-200' : 'text-slate-600 hover:text-slate-900'}`
              }`}
            >
              Investor
      ...
      <nav className="flex-1 overflow-y-auto px-3 py-4">
        {currentNavigation.map((section, sectionIndex) => (
          <div key={sectionIndex} className="mb-6">
            {!collapsed && (
              <h3 className={`px-3 mb-2 text-xs font-semibold ${isDarkMode ? 'text-slate-500' : 'text-slate-500'} uppercase tracking-wider`}>
                {section.title}
              </h3>
            )}
              ...
              {section.items.map((item, itemIndex) => (
                <Link
                  ...
                  className={`flex items-center gap-3 px-3 py-2.5 rounded-lg transition-all group relative ${
                    isActive(item.path)
                      ? `${isDarkMode ? 'text-white' : 'text-white'} shadow-md`
      ...
      <div className={`border-t ${isDarkMode ? 'border-slate-800' : 'border-slate-200'} p-3`}>
        <Link
          to="/"
          className={`flex items-center gap-3 px-3 py-2.5 rounded-lg ${isDarkMode ? 'text-slate-300 hover:bg-slate-800' : 'text-slate-700 hover:bg-slate-100'} transition-all`}
        >
